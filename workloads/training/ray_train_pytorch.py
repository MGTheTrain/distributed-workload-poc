...
class Config:
    ...
    # Training
    LEARNING_RATE = float(
        os.getenv("LEARNING_RATE", "0.001541")
    )  # Changed from 0.001 (tuning found 0.001541)
    BATCH_SIZE = int(
        os.getenv("BATCH_SIZE", "32")
    )  # Changed from 64 (tuning found 32 is optimal)
    EPOCHS = int(
        os.getenv("EPOCHS", "10")
...
def train_func(config: Dict[str, Any]) -> None:
    ...
    train_dataset = datasets.MNIST(
        root=str(Config.DATA_DIR), train=True, download=False, transform=transform
    )
    test_dataset = datasets.MNIST(
        root=str(Config.DATA_DIR), train=False, download=False, transform=transform
    )
    ...
    for epoch in range(epochs):
        ...
        if world_size > 1:
            ...
            train_loss_tensor = torch.tensor(
                avg_train_loss * len(train_loader), dtype=torch.float32
            )
            ...
            torch.distributed.all_reduce(
                test_loss_tensor, op=torch.distributed.ReduceOp.SUM
            )
            torch.distributed.all_reduce(
                correct_tensor, op=torch.distributed.ReduceOp.SUM
            )
            torch.distributed.all_reduce(
                total_tensor, op=torch.distributed.ReduceOp.SUM
            )
        ...
        if is_rank_0 and mlflow_tracking_uri and mlflow_run_id:
            try:
                ...
                client.log_metric(
                    mlflow_run_id, "train_loss", avg_train_loss, step=epoch
                )
