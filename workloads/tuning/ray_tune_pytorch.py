...
class Config:
    ...
    MAX_CONCURRENT_TRIALS = int(
        os.getenv("MAX_CONCURRENT_TRIALS", "3")
    )  # Limit parallel trials to prevent OOM
    ...
    with mlflow.start_run(run_name="hyperparameter_tuning") as parent_run:
        ...
        try:
            ...
            search_alg = ConcurrencyLimiter(
                search_alg, max_concurrent=Config.MAX_CONCURRENT_TRIALS
            )
        ...
        print("\n Search space:")
        print("   Learning rate: 1e-4 to 1e-1 (log-uniform)")
        ...
        if best_result.checkpoint:
            with best_result.checkpoint.as_directory() as checkpoint_dir:
                ...
                for file in Path(checkpoint_dir).rglob("*"):
                    if file.is_file():
                        dest = Path(local_checkpoint_dir) / file.relative_to(
                            checkpoint_dir
                        )
