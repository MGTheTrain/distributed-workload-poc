...
class Config:
    ...
    OUTPUT_FILE = Path(
        os.getenv("OUTPUT_FILE", str(DATA_DIR / "processed_sales_ray_data.parquet"))
    )
...
def upload_to_s3(s3_client, local_file: Path, bucket: str, s3_key: str) -> str:
...
def generate_sales_data(num_rows: int, output_file: Path) -> None:
    ...
    data = {
        "product_id": [
            f"PROD_{i:06d}" for i in np.random.randint(1000, 9999, num_rows)
        ],
...
def run_etl_pipeline() -> None:
    ...
    try:
        ...
        agg_ds = ds_transformed.groupby(["category", "year", "quarter"]).aggregate(
            Sum("total_revenue"), Mean("total_revenue"), Count(), Sum("quantity")
        )
        ...
        print(
            f" Materialized {output_rows:,} aggregated rows in {materialize_time:.2f}s"
        )
        ...
        s3_uri = upload_to_s3(s3_client, Config.OUTPUT_FILE, Config.S3_BUCKET, s3_key)

        # Also upload a "latest" version for easy access
        latest_s3_key = f"{Config.S3_PREFIX}/processed_sales_latest.parquet"
        upload_to_s3(s3_client, Config.OUTPUT_FILE, Config.S3_BUCKET, latest_s3_key)
