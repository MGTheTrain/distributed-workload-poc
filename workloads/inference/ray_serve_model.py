...
class MNISTInferenceAPI:
    def __init__(
        self, model_source: str = "mlflow", model_identifier: Optional[str] = None
    ):
    ...
    def _load_model(self) -> Path:
        if self.model_source == "mlflow":
            return (
                load_model_from_mlflow(self.model_identifier)
                if self.model_identifier
                else get_best_model_from_mlflow(Config.MLFLOW_EXPERIMENT)
            )
        elif self.model_source == "local":
            path = (
                Path(self.model_identifier)
                if self.model_identifier
                else Config.MODEL_DIR / "model.pt"
            )
            if not path.exists():
...
if Config.RUN_ID:
    deployment = MNISTInferenceAPI.bind(
        model_source="mlflow", model_identifier=Config.RUN_ID
    )
else:
